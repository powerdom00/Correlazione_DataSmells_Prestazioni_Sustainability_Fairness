...
def load_processed_dataset():
    ...
    df = utils.open_dataset("src/datasets/Compas Dataset/compas_processed.csv")
    return df
...
def run_model_training():
    ...
    for i in range(30):
        ...
        for keys, item in results.items():
            utils.to_csv("src/output/reports/compas/compas_reports.csv", item, "compas")
...
def training_model(dataset, index, process_results):
    ...
    pickle.dump(lr_model_pipeline, open(f'src/output/standard_models/compas/lr_model_{index}.sav', 'wb'))
    pickle.dump(rf_model_pipeline, open(f'src/output/standard_models/compas/rf_model_{index}.sav', 'wb'))
    pickle.dump(xgb_model_pipeline, open(f'src/output/standard_models/compas/xgb_model_{index}.sav', 'wb'))
    pickle.dump(svm_model_pipeline, open(f'src/output/standard_models/compas/svm_model_{index}.sav', 'wb'))
    ...
    lr_results = {
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/compas/lr_model_{index}.sav")
    }
    rf_results = {
        'dataset': 'compas',
        'iteration': index + 1,
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/compas/rf_model_{index}.sav")
    }
    svm_results = {
        'dataset': 'compas',
        'iteration': index + 1,
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/compas/svm_model_{index}.sav")
    }
    xgb_results = {
        'dataset': 'compas',
        'iteration': index + 1,
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/compas/xgb_model_{index}.sav")
    }
...
def eq_odds_fair_report(dataset, prediction, results):
    ...
    # Create BinaryLabelDataset for overall protected attributes
    ...
    # Calculate fairness metrics for overall protected attributes
