...
def load_processed_dataset():
    ...
    df = utils.open_dataset("src/datasets/fpes/fpes.csv")
    return df
...
def run_model_training():
    ...
    for i in range(30):
        ...
        for keys, item in results.items():
            utils.to_csv("src/output/reports/fpes/fpes_reports.csv", item, "fpes")
...
def training_model(dataset, index, process_results):
    ...
    pickle.dump(lr_model_pipeline, open(f'src/output/standard_models/fpes/lr_model_{index}.sav', 'wb'))
    pickle.dump(rf_model_pipeline, open(f'src/output/standard_models/fpes/rf_model_{index}.sav', 'wb'))
    pickle.dump(xgb_model_pipeline, open(f'src/output/standard_models/fpes/xgb_model_{index}.sav', 'wb'))
    pickle.dump(svm_model_pipeline, open(f'src/output/standard_models/fpes/svm_model_{index}.sav', 'wb'))
    ...
    lr_results = {
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/fpes/lr_model_{index}.sav")
    }
    rf_results = {
        'dataset': 'fpes',
        'iteration': index + 1,
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/fpes/rf_model_{index}.sav")
    }
    svm_results = {
        'dataset': 'fpes',
        'iteration': index + 1,
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/fpes/svm_model_{index}.sav")
    }
    xgb_results = {
        'dataset': 'fpes',
        'iteration': index + 1,
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/fpes/xgb_model_{index}.sav")
    }
    ...
    X_test_df['promotion'] = y_test

    lr_pred = X_test_df.copy(deep=True)
    lr_pred['promotion'] = lr_model_pipeline.predict(X_test)

    rf_pred = X_test_df.copy(deep=True)
    rf_pred['promotion'] = rf_model_pipeline.predict(X_test)

    svm_pred = X_test_df.copy(deep=True)
    svm_pred['promotion'] = svm_model_pipeline.predict(X_test)

    xgb_pred = X_test_df.copy(deep=True)
    xgb_pred['promotion'] = xgb_model_pipeline.predict(X_test)
...
def eq_odds_fair_report(dataset, prediction, results):
    ...
    race_feature = ['race_B','race_H','race_W']

    # Create BinaryLabelDataset for race
    ...
    race_privileged_groups = [{'race_W': 1.0}]
    race_unprivileged_groups = [{'race_W': 0.0}]
