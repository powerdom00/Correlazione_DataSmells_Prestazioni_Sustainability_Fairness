...
def load_processed_dataset():
    ...
    df = utils.open_dataset("src/datasets/diabetic/diabetic.csv")
    return df
...
def run_model_training():
    ...
    for i in range(30):
        ...
        for keys, item in results.items():
            utils.to_csv("src/output/reports/diabetic/diabetic_reports.csv", item, "diabetic")
...
def training_model(dataset, index, process_results):
    ...
    pickle.dump(lr_model_pipeline, open(f'src/output/standard_models/diabetic/lr_model_{index}.sav', 'wb'))
    pickle.dump(rf_model_pipeline, open(f'src/output/standard_models/diabetic/rf_model_{index}.sav', 'wb'))
    pickle.dump(xgb_model_pipeline, open(f'src/output/standard_models/diabetic/xgb_model_{index}.sav', 'wb'))
    pickle.dump(svm_model_pipeline, open(f'src/output/standard_models/diabetic/svm_model_{index}.sav', 'wb'))
    ...
    lr_results = {
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/diabetic/lr_model_{index}.sav")
    }
    rf_results = {
        'dataset': 'diabetic',
        'iteration': index + 1,
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/diabetic/rf_model_{index}.sav")
    }
    svm_results = {
        'dataset': 'diabetic',
        'iteration': index + 1,
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/diabetic/svm_model_{index}.sav")
    }
    xgb_results = {
        'dataset': 'diabetic',
        'iteration': index + 1,
        ...
        'size': utils.get_mb_size(f"src/output/standard_models/diabetic/xgb_model_{index}.sav")
    }
    ...
    X_test_df['readmitted'] = y_test

    lr_pred = X_test_df.copy(deep=True)
    lr_pred['readmitted'] = lr_model_pipeline.predict(X_test)

    rf_pred = X_test_df.copy(deep=True)
    rf_pred['readmitted'] = rf_model_pipeline.predict(X_test)

    svm_pred = X_test_df.copy(deep=True)
    svm_pred['readmitted'] = svm_model_pipeline.predict(X_test)

    xgb_pred = X_test_df.copy(deep=True)
    xgb_pred['readmitted'] = xgb_model_pipeline.predict(X_test)
...
def eq_odds_fair_report(dataset, prediction, results):
    ...
    aif_sex_dataset = BinaryLabelDataset(
        ...
        privileged_protected_attributes=['gender_Male'],
    )
    ...
    sex_privileged_groups = [{'gender_Male': 1.0}]
    sex_unprivileged_groups = [{'gender_Male': 0.0}]
    ...
    race_features = ['race_?', 'race_AfricanAmerican', 'race_Asian', 'race_Caucasian', 'race_Hispanic', 'race_Other']
    ...
    aif_race_dataset = BinaryLabelDataset(
        ...
        privileged_protected_attributes=['race_Caucasian'],
    )
    ...
    race_privileged_groups = [{'race_Caucasian': 1.0}]
    race_unprivileged_groups = [{'race_Caucasian': 0.0}]
    ...
    privileged_groups = [{'gender_Male': 1.0} | {'race_Caucasian': 1.0}]
    unprivileged_groups = [{'gender_Male': 0.0, 'race_Caucasian': 0.0}]
